...
//			this.OnDeactivate();
		...
		public override void OnManagedCreateResources()
		{
			...
			this.txTopPanel2 = CDTXMania.tGenerateTexture(CSkin.Path(@"Graphics\5_header song listGF.png"), false);
...
//							int x = 720 - ( (int) ( width * db回転率 ) );
						...
						if (CDTXMania.ConfigIni.bGuitarEnabled)
							this.txTopPanel2.tDraw2D(CDTXMania.app.Device, 0f, ((float)(this.txTopPanel2.szTextureSize.Height) * ((float)(this.ct登場アニメ用[0].nCurrentValue) / 100f)) - (float)(this.txTopPanel2.szTextureSize.Height));
...
//					int x = this.ptバーの基本座標[ n見た目の行番号 ].X + ( (int) ( ( this.ptバーの基本座標[ n次のパネル番号 ].X - this.ptバーの基本座標[ n見た目の行番号 ].X ) * ( ( (double) Math.Abs( this.nCurrentScrollCounter ) ) / 100.0 ) ) );
				...
				if (CDTXMania.ConfigIni.bGuitarEnabled)
					this.txTopPanel2.tDraw2D(CDTXMania.app.Device, 0, 0);
		...
		private CTexture txTopPanel2;           // tx上部パネル(Guitar&Bass)
